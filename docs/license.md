---
layout: default
title: Get a Supporter License
lang: en
page_id: license
---

<h1 style="margin-top: 2em;">NumiSync Supporter License</h1>

## What's Included

NumiSync Wizard is **free for core features**, but a Supporter License ($10) unlocks premium features and removes nag prompts.

### Free Features (Always Available)

- Intelligent coin matching and search
- Visual field comparison and selective updates
- Issue variant selection
- Image download and comparison
- Smart caching and multi-machine support
- Advanced field mapping

### Supporter License Features ($10)

- **Fast Pricing Mode** - Batch update pricing across your entire collection in seconds
- **Auto-Propagate** - Automatically apply type data to all matching coins
- **No more nag prompts** - Uninterrupted workflow
- **Discounts on future premium features** - Early supporter benefits
- **Support independent software** - Help keep NumiSync improving

**Buy once, use forever** • 5-device activation limit • No subscriptions

---

## Purchase a License

<div style="text-align: center; margin: 2em 0;">
  <a href="https://buy.polar.sh/polar_cl_4hKjIXXM8bsjk9MivMFIvtXbg7zWswAzEAVJK2TVZZ0" target="_blank" rel="noopener" onclick="gtag('event', 'purchase_click', { product: 'supporter_license' })" style="display: inline-block; padding: 16px 32px; background: #6f42c1; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 1.2em;">Purchase Now - $10</a>
</div>

<div style="text-align: center; margin: 1em 0; color: #666;">
  Secure checkout via Polar.sh
</div>

After purchase, you'll receive a license key via email.

---

## Activate Your License

### Step 1: Copy Your License Key

After purchasing, you'll receive an email from Polar.sh with your license key. It will look like:

```
V1-XXXX-XXXX-XXXX-XXXX
```

### Step 2: Open NumiSync Wizard

Launch the NumiSync Wizard application on your computer.

### Step 3: Enter License Key

1. Click **Settings** in the menu bar (or press `Ctrl+,` / `Cmd+,`)
2. Navigate to the **License** tab
3. Paste your license key into the text field
4. Click **Activate**

### Step 4: Verify Activation

Once activated, you'll see:
- ✅ License status: **Active**
- Premium features unlocked immediately
- No more nag prompts

---

## Frequently Asked Questions

### Can I use my license on multiple computers?

Yes! You can activate your license on up to **5 devices**. Perfect for home, work, laptop, and backup machines.

### Is this a subscription?

No! **Buy once, use forever.** Your license never expires and includes all features from version 1.x.

### What about future major versions (v2.0, v3.0)?

Your v1 license unlocks all v1.x features permanently. Future major versions (v2.0+) with significant new features may require a separate license, but existing v1 features will continue working. Early supporters will receive upgrade discounts.

### I lost my license key. Can I retrieve it?

Yes! Your license key is tied to your email address. Check your purchase confirmation email from Polar.sh, or contact support with your purchase email.

### Does this support the developer?

Absolutely! Your $10 helps cover:
- Domain hosting and SSL certificates
- API infrastructure and caching servers
- Development time for new features
- Code signing certificates (when available)
- Coffee for late-night debugging sessions ☕

Every supporter license directly funds NumiSync's continued development and keeps it free for basic use.

---

## Need Help?

- **Installation issues?** See the [Installation Guide](/installation)
- **Questions about features?** Check the in-app documentation (press F1)
- **Technical support?** [Open an issue on GitHub](https://github.com/inguy24/numismat-enrichment/issues)
- **Other questions?** Email: [shane@shaneburkhardt.com](mailto:shane@shaneburkhardt.com)

---

<div style="text-align: center; margin: 3em 0;">
  <a href="/" style="display: inline-block; padding: 10px 20px; background: #0066cc; color: white; text-decoration: none; border-radius: 6px;">Back to Home</a>
</div>
