...
33. **All external links MUST open in the system browser — never inside the app** — In Electron, `<a href="..." target="_blank">` does NOT open the system browser; it opens a new `BrowserWindow` inside the app (an embedded browser with no preload/context isolation). This is both a security risk and broken UX. **Two-layer enforcement is required:** (1) `setWindowOpenHandler` on every `BrowserWindow` instance (main window, manual window, any future windows) — this is the safety net that intercepts all `target="_blank"` clicks and routes them to `shell.openExternal()`; add it immediately after `window.loadFile()`. (2) In renderer HTML that has preload access (`index.html`), convert external links from `target="_blank"` to `onclick="window.electronAPI.openExternal('...'); return false;"`. In HTML without preload access (`user-manual.html`), keep `target="_blank"` — the `setWindowOpenHandler` covers it. **Visual indicator required:** every external link must signal it opens outside the app — use CSS class `ext-link` (in `main.css`) for preload-accessible HTML and `a[target="_blank"]::after` CSS (inline `<style>`) for standalone HTML windows. Both render a `↗` arrow via `content: "\2197"`. Always add `title="Opens in your browser"` to external links for accessibility. **Never use bare `<a href="https://...">` with no onclick/target** — this navigates the current window away from the app entirely.
...
21. **Price fields are driven by the field mapping config — never hardcode the grade→column mapping** — `price1=F, price2=VF, price3=XF, price4=UNC` is the correct mapping (confirmed Feb 2026: `SELECT price1 FROM coins WHERE id=523` returns the Fine value). But the *mechanism* that resolves this mapping is the configurable field mapping system, not any hardcoded constant or static method. Each price field has a `numistaPath` (e.g., `'pricing.f'` for `price1`) set in `DEFAULT_FIELD_MAPPING` and overrideable by user settings. Before the field loop in `mapToOpenNumismat()`, pricingData is merged into `dataForLookup` as `{ pricing: { f, vf, xf, unc } }` so `getNestedValue(dataForLookup, 'pricing.f')` resolves correctly — the same standard path used by every other field. **Never add a special-case branch or static method that bypasses `config.numistaPath` for price fields.** That was the root cause of two incidents: (1) Feb 9 2026 — `fast-pricing-update` had correct mapping but was "fixed" to match a wrong `field-mapper.js`; (2) Feb 23 2026 — `extractPriceFields()` hardcoded the mapping as a static method, making the source dropdowns in Field Mappings cosmetic (the user could change them but they had zero effect). Both times trusting existing code over the actual DB column was the error. The UI displays grades descending (UNC at top-left, Fine at bottom-right) — the **opposite** of column numbering — never infer the mapping from display order.

34. **Never bypass the field mapping system with hardcoded alternatives** — The app has a configurable field mapping system: each OpenNumismat column has a `numistaPath` (and optional `transform`) that the user can reconfigure via the Field Mappings settings screen. Any code that hardcodes which Numista value goes to which column — instead of reading `config.numistaPath` from the field mapping config — makes the settings UI a lie. The correct pattern for all fields, including price fields: (1) `buildFieldMapperConfig()` produces the FieldMapper config from user settings; (2) `mapToOpenNumismat()` iterates fields and calls `getNestedValue(dataForLookup, config.numistaPath)` for each one; (3) source data that comes from a separate API call (e.g., pricing) is merged into `dataForLookup` before the loop so the same `getNestedValue` path works. Never add a static method, a hardcoded map literal, or a special-case `else if` branch that substitutes for reading `config.numistaPath`.

35. **When data is fetched for a field category, write ALL fields in that category — null for missing values** — When `pricingData` is fetched, all four price columns must be written on every update. If Numista returns no Fine price, `price1` must be written as `null` — not skipped. Skipping means a stale value (e.g., a wrong price from a previous buggy enrichment) stays in the column indefinitely. The rule: `requiresPricingData` fields are always included in `mapped` when `pricingData !== null`, even if `value` is null. The same principle applies to any future field category that comes from a separate API call: fetch the data → write all fields → null for absent values. In `fast-pricing-update`, this means iterating `['price1','price2','price3','price4']`, resolving each field's `numistaPath` from `buildFieldMapperConfig()`, and writing `grades[gradeKey] ?? null` for all four — never just the grades that happened to be returned.
